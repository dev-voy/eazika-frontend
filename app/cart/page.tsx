...
import { useCartStore, CartItem as CartItemType } from '@/app/hooks/useCartStore';
...
const CartItem = ({ 
    ...
    return (
        <div className="relative rounded-2xl shadow-sm overflow-hidden">
            <div className="absolute inset-y-0 right-0 w-20 bg-gray-800 flex items-center justify-center">
            ...
            <motion.div
                ...
                className="relative bg-white p-3 flex items-center gap-3 z-10"
            >
                <div className="relative w-20 h-20 rounded-2xl overflow-hidden flex-shrink-0">
                    <Image src={item.image} alt={item.name} fill className="object-cover" />
                    <div className="absolute bottom-0 left-0 bg-yellow-400 text-gray-800 text-xs font-bold px-2 py-1 rounded-tr-xl">
                        {item.price}
                    </div>
                </div>
                ...
                <div className="flex items-center space-x-2 bg-gray-100 border border-gray-200 rounded-full px-2 py-1">
...
export default function CartPage() {
  ...
  return (
    <MainLayout>
      <div className="w-full max-w-7xl mx-auto bg-gray-50 h-screen flex flex-col">
        <header className="px-4 md:px-6 py-4 flex items-center space-x-4 border-b bg-white sticky top-0 z-20 flex-shrink-0">
            {/* CORRECTED LINK */}
        ...
        <div className="flex-grow overflow-y-auto lg:grid lg:grid-cols-3 lg:gap-8">
            <main className="lg:col-span-2 p-4 md:p-6">
                {cartItems.length === 0 ? (
            ...
            {cartItems.length > 0 && (
                <aside className="hidden lg:block lg:sticky lg:top-24 h-fit p-6">
                    <OrderSummary />
                </aside>
            )}
        </div>
        
        {/* Mobile Summary: Fixed at the bottom, outside the scrolling area */}
        {cartItems.length > 0 && (
            <footer className="lg:hidden bg-white p-4 border-t fixed bottom-16 left-0 right-0 w-full flex-shrink-0 z-30">
